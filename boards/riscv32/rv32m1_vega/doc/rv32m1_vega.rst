...
.. warning::
   ...
   The OpenSDA chip cannot be used to debug the RISC-V cores.
...
First, connect the J-Link debugger through the adapter board to the
...
Second, power the VEGAboard via USB. The OpenSDA connector at the top
left is recommended for UART access.

Third, make sure your J-Link is connected to your computer via USB.
...
the ``ri5cy_boot`` command **with the reset button on the board (at
top left) pressed down**::
...
#. run ``init`` and ``ri5cy_boot`` as shown above, and quit the OpenOCD
#. and telnet sessions.
...
by using :ref:`west flash <west-flash-debug>` instead of ``ninja
...
.. note::
   ...
   Because this currently flashes a binary file and due to details of
   the way RI5CY images are linked, this can take a lot longer than
   debugging with GDB and running ``load``.
...
From the GDB session, you can load the program with ``load`` and
execute OpenOCD commands such as ``monitor reset halt``.

.. note::

   GDB can get out of sync with the target if you execute commands
   that reset it. To reset RI5CY and get GDB back in sync with it, use
   this sequence:
...
For manual steps to run OpenOCD and GDB to debug, see the `SDK README`_.

References
**********

- OpenISA developer portal: http://open-isa.org
- `OpenISA GitHub releases`_: includes documentation, such as the SoC
  datasheet and reference manual, board schematic and user guides, etc.
- Base toolchain: `pulp-riscv-gnu-toolchain`_; extra toolchain patches:
  `rv32m1_gnu_toolchain_patch`_
- OpenOCD repository: `rv32m1-openocd`_
