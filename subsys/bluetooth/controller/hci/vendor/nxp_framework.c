...
typedef struct mem_manager_pools {
...
#define MM_POOLS_CNT  sizeof(mm_pools) / sizeof(mm_pools[0])
...
#define MEM_USAGE_STATS

#ifdef MEM_USAGE_STATS
...
void MEM_StatAlloc(mem_manager_pools_t *pool, uint32_t numBytes)
{
	pool->curr++;
 ...
 */
tmrTimerTableEntry_t tmr_table[CONFIG_KW41_NUM_TIMERS];
 ...
 *
 * @return 0 if encryption/decryption was successfull; otherwise, error
...
)
{
	...
	if (uECC_make_key((uint8_t *)pOutPublicKey,
		...
		SYS_LOG_ERR("ECDH_P256_GenerateKeys: error?!?!?");
		return gSecError_c;
	}
}
...
)
{
	...
	if (uECC_shared_secret((const uint8_t*)pPeerPublicKey,
		...
		SYS_LOG_ERR("ECDH_P256_ComputeDhKey: error?!?!?\n");
		return gSecError_c;
	}
}
