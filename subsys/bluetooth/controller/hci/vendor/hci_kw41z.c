...
#define SYS_LOG_DOMAIN "BLE_KW41Z"
...
static void ble_decode_cmd(struct net_buf *buf)
{
	...
	printk("ble_decode_cmd: Hci_SendPacketToController(type: HCI_CMD (%d), len: %d):\n", BT_BUF_CMD, buf->len);
 ...
 */
static struct k_thread kw41_controller_thread_data;
K_THREAD_STACK_DEFINE(kw41_controller_stack, CONFIG_KW41_CTRL_STACK_SIZE);
...
static bleResult_t kw41z_hci_recv(hciPacketType_t type, void *pPacket, uint16_t packetSize)
{
	...
	SYS_LOG_DBG("type: 0x%02X, evt_hdr->evt: 0x%02X, evt_hdr->len: %d", type, evt_hdr->evt, evt_hdr->len);
...
static int Controller_TaskInit(void)
{
	...
	taskID = k_thread_create(
		&kw41_controller_thread_data,
		kw41_controller_stack,
		CONFIG_KW41_CTRL_STACK_SIZE,
		ControllerTask,
		NULL, NULL, NULL,
	...
	IRQ_CONNECT(Radio_0_IRQn, RADIO_0_IRQ_PRIO, Controller_InterruptHandler, 0, 0);
