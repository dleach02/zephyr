 ...
 * Coexstence support
 *
 * The RV32M1 link layer library uses this API to determine if usage of
 * the radio is allowed. Additional framework is required to more fully
 * realize coexistence on Zephyr & RV32M1.
 *
 * @return  uint8_t !CONFIG_RV32M1_COEX_RF_DENY_ACTIVE_STATE - RF Deny Inactive,
 *                   CONFIG_RV32M1_COEX_RF_DENY_ACTIVE_STATE - RF Deny Active
 *
 * ---------------------------------------------------------------------
 */
uint8_t MWS_CoexistenceDenyState(void)
{
	uint8_t retVal = !CONFIG_RV32M1_COEX_RF_DENY_ACTIVE_STATE;
	...
	if (rf_deny) {
		if (GpioReadPinInput(rf_deny) ==
		    CONFIG_RV32M1_COEX_RF_DENY_ACTIVE_STATE) {
			retVal = CONFIG_RV32M1_COEX_RF_DENY_ACTIVE_STATE;
		}
	}
#endif
	return retVal;
...
#if (CONFIG_RV32M1_POOL_32_CNT > 0)
MEM_MANAGER_POOL(32, CONFIG_RV32M1_POOL_32_CNT);
#endif
#if (CONFIG_RV32M1_POOL_64_CNT > 0)
MEM_MANAGER_POOL(64, CONFIG_RV32M1_POOL_64_CNT);
#endif
#if (CONFIG_RV32M1_POOL_128_CNT > 0)
MEM_MANAGER_POOL(128, CONFIG_RV32M1_POOL_128_CNT);
#endif
#if (CONFIG_RV32M1_POOL_256_CNT > 0)
MEM_MANAGER_POOL(256, CONFIG_RV32M1_POOL_256_CNT);
#endif
#if (CONFIG_RV32M1_POOL_512_CNT > 0)
MEM_MANAGER_POOL(512, CONFIG_RV32M1_POOL_512_CNT);
#endif
#if (CONFIG_RV32M1_POOL_1024_CNT > 0)
MEM_MANAGER_POOL(1024, CONFIG_RV32M1_POOL_1024_CNT);
#endif
...
#if (CONFIG_RV32M1_POOL_32_CNT > 0)
	{ &mm_pool_32, 32, CONFIG_RV32M1_POOL_32_CNT, 0, 0, 0 },
#endif
#if (CONFIG_RV32M1_POOL_64_CNT > 0)
	{ &mm_pool_64, 64, CONFIG_RV32M1_POOL_64_CNT, 0, 0, 0 },
#endif
#if (CONFIG_RV32M1_POOL_128_CNT > 0)
	{ &mm_pool_128, 128, CONFIG_RV32M1_POOL_128_CNT, 0, 0, 0 },
#endif
#if (CONFIG_RV32M1_POOL_256_CNT > 0)
	{ &mm_pool_256, 256, CONFIG_RV32M1_POOL_256_CNT, 0, 0, 0 },
#endif
#if (CONFIG_RV32M1_POOL_512_CNT > 0)
	{ &mm_pool_512, 512, CONFIG_RV32M1_POOL_512_CNT, 0, 0, 0 },
#endif
#if (CONFIG_RV32M1_POOL_1024_CNT > 0)
	{ &mm_pool_1024, 1024, CONFIG_RV32M1_POOL_1024_CNT, 0, 0, 0 },
#endif
};
...
#ifdef CONFIG_RV32M1_MEM_USAGE_STATS
	...
	void *pCaller)
{
	...
	if (rslt) {
		SYS_LOG_DBG("%p, pool: %d, bufSize: %" PRIu32 ", numBytes: %" PRIu32 "", rslt, i, mm_pools[i].size, numBytes);
		MEM_StatAlloc(&mm_pools[i], numBytes);
	} else {
		SYS_LOG_ERR("ERROR! Alloc request failure!, pool: %d, numBytes: %" PRIu32 "", i, numBytes);
		MEM_PrintInfo();
	}
...
/*
 * The RV32M1 Link Layer library require one timer. A Kconfig value is used
 * in case this changes in the future.
 */
struct tmrTimerTableEntry tmr_table[CONFIG_RV32M1_NUM_TIMERS];
 ...
 *
 */
enum tmrErrCode_tag TMR_StopTimer(tmrTimerID_t timerID)
{
	enum tmrErrCode_tag err = gTmrSuccess_c;

	SYS_LOG_DBG("ID: %d", timerID);
 ...
 */
enum tmrErrCode_tag TMR_FreeTimer(tmrTimerID_t timerID)
{
	enum tmrErrCode_tag status;
 ...
 *
 */
enum tmrErrCode_tag TMR_StartTimer(
...
)
{
	...
	SYS_LOG_DBG("ID: %d, ms: %ld", timerID, timeInMilliseconds);
