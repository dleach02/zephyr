 ...
 * -----------------------------------------------------------------------------
 *
 * HCI Driver Interface functions to the RV32M1 BLE link layer controller library
 * as defined by Zephyr.
 *
 * -----------------------------------------------------------------------------
 */

#define SYS_LOG_LEVEL CONFIG_SYS_LOG_BT_HCI_DRIVER_LEVEL
#define SYS_LOG_DOMAIN "BLE_RV32M1"
 ...
 * RV32M1 Link Layer Controller Library
 */
...
const uint8_t gAdvertisingPacketInterval_c =
	CONFIG_RV32M1_ADVERTISING_PACKET_INTERVAL;
 ...
 */
const uint8_t gScanChannelMap_c = CONFIG_RV32M1_SCAN_CHANNEL_MAP;
 ...
 * 0x04 - Enables channel 39 for use.
 */
const uint8_t gInitiatorChannelMap_c = CONFIG_RV32M1_INITIATOR_CHANNEL_MAP;
 ...
 */
const uint16_t gOffsetToFirstInstant_c = CONFIG_RV32M1_OFFSET_TO_FIRST_INSTANT;

/* Radio system clock selection. */
#if (RF_OSC_26MHZ == 1)
const uint8_t gRfSysClk26MHz_c = 1;  /* 26MHz radio clock. */
#else
const uint8_t gRfSysClk26MHz_c;      /* 32MHz radio clock. (value == 0) */
#endif
 ...
 */
static struct k_thread rv32m1_controller_thread_data;
K_THREAD_STACK_DEFINE(rv32m1_controller_stack, CONFIG_RV32M1_CTRL_STACK_SIZE);
...
static K_THREAD_STACK_DEFINE(rx_prio_thread_stack, CONFIG_BT_RX_STACK_SIZE);
static struct k_thread rx_prio_thread_data;
 ...
 * function defined in the RV32M1 BLE link layer controller library.
 */
 ...
 */
static bleResult_t rv32m1_hci_recv(hciPacketType_t type, void *pPacket, uint16_t packetSize)
{
	...
	SYS_LOG_DBG("type: 0x%02X, evt_hdr->evt: 0x%02X, evt_hdr->len: %d",
		    type, evt_hdr->evt, evt_hdr->len);
	_hexdump(pPacket, packetSize);
...
static int rv32m1_send(struct net_buf *buf)
{
	u8_t type = 0;
	int err = 0;
	...
	if (!type) {
		SYS_LOG_ERR("Unknown BT buff type: 0x%X", bt_buf_get_type(buf));
		net_buf_unref(buf);
	} else {
		SYS_LOG_DBG("Hci_SendPacketToController(type: gHciDataPacket_c (%d), len: %d):", type, buf->len);
		_hexdump(buf->data, buf->len);
...
static int rv32m1_open(void)
{
	...
	k_thread_create(&rx_prio_thread_data, rx_prio_thread_stack,
			K_THREAD_STACK_SIZEOF(rx_prio_thread_stack),
...
static const struct bt_hci_driver rv32m1_ble_drv = {
	.name   = "RV32M1_BT42",
...
/*
 * DJLZORK: This should be setup by the dtsi (see GPIOB example).
 */
#include "dt-bindings/interrupt-controller/openisa-intmux.h"
#define RF_IRQ INTMUX_LEVEL2_IRQ(INTMUX_CH0, 29)

#if (defined(CPU_RV32M1_ri5cy) || defined(CPU_RV32M1_cm0plus))
...
/*
 * RV32M1 BLE link layer controller task initialization.
 */
static int Controller_TaskInit(void)
{
	...
	taskID = k_thread_create(
		&rv32m1_controller_thread_data,
		rv32m1_controller_stack,
		CONFIG_RV32M1_CTRL_STACK_SIZE,
		ControllerTask,
		NULL, NULL, NULL,
		K_PRIO_PREEMPT(CONFIG_RV32M1_CTRL_PRIO),
		0, K_NO_WAIT);
/* DJLZORK */
	...
	IRQ_CONNECT(RF_IRQ, 1, RF0_0_IRQHandler,0,0);
	irq_enable(RF_IRQ);
	...
	Controller_SetTxPowerLevel(
		CONFIG_RV32M1_ADVERTISING_DEFAULT_TX_POWER, gAdvTxChannel_c);
	Controller_SetTxPowerLevel(
		CONFIG_RV32M1_CONNECTION_DEFAULT_TX_POWER, gConnTxChannel_c);

	return osaStatus_Success;
}

static int _bt_rv32m1_init(struct device *unused)
{
	ARG_UNUSED(unused);
	...
	XCVR_SetXtalTrim(CONFIG_RV32M1_XTAL_TRIM);
	...
	gMCUSleepDuringBleEvents = CONFIG_RV32M1_MCU_SLEEP_DURING_BLE_EVENTS;

	/* Configure BD_ADDR before calling the Controller_Init */

	/* DJLZORK: RSIM doesn't have MAC_MSB so need to use something else */
...
/*
	...
	SYS_LOG_DBG("Set BD_ADDR: ");
	_hexdump(&gBD_ADDR[0], 6);

	/* BLE Controller Init */
	if (osaStatus_Success != Controller_Init(rv32m1_hci_recv)) {
		SYS_LOG_ERR("ERROR initializing controller");
		return gBleOsError_c;
	}

	bt_hci_driver_register(&rv32m1_ble_drv);
	return 0;
}

SYS_INIT(_bt_rv32m1_init, POST_KERNEL, CONFIG_KERNEL_INIT_PRIORITY_DEVICE);
